...
<div class="card shadow">
          ...
          <div class="card-body">
             ...
             <form action="{{ url('appointments') }}" method="POST">
                 ...
                 <input name="description" value="{{ old('description') }}" type="text" id="description" class="form-control" placeholder="Describe brevemente la consulta" required>
                 </div>
                  ...
                  <div class="form-group">
                     ...
                     @if($intervals)
                      ...
                      <div class="custom-control custom-radio mb-3">
                        <input type="radio" id="intervalMorning{{ $key }}" name="schedule_time" value="{{ $interval['start'] }}" class="custom-control-input" required>
                        <label class="custom-control-label" for="intervalMorning{{ $key }}">{{ $interval['start'] }} - {{ $interval['end'] }} </label>
                      </div>
                      @endforeach
                      @foreach($intervals['afternoon'] as $key => $interval)
                      <div class="custom-control custom-radio mb-3">
                        <input type="radio" id="intervalAfternoon{{ $key }}" name="schedule_time" value="{{ $interval['start'] }}" class="custom-control-input" required>
